...
def generate_subspace_mog(num_samples: int, intrinsic_dim: int, ambient_dim: int, seed=999, num_modes: int = 8) -> torch.Tensor:
    """
    Generates a Mixture of Gaussians (MoG) strictly confined to a d-dimensional subspace.
    Tests the score network's ability to handle low-density regions between modes.
    """
    torch.manual_seed(42) 
    centers = torch.randn(num_modes, intrinsic_dim) * 3.0

    idx = torch.randint(0, num_modes, (num_samples,))
    x_intrinsic = centers[idx] + 0.2 * torch.randn(num_samples, intrinsic_dim)

    # Standardize to keep the domain well-behaved
    x_intrinsic = x_intrinsic - x_intrinsic.mean(dim=0)
    x_intrinsic = x_intrinsic / torch.std(x_intrinsic)

    padding = torch.zeros(num_samples, ambient_dim - intrinsic_dim)
    x_embedded = torch.cat([x_intrinsic, padding], dim=1)
...
if __name__ == "__main__":
    ...
    for run in range(num_runs):
        ...
        for i, n in enumerate(tqdm(n_values, desc=f"Evaluating Sizes (Run {run+1})")):
            ...
            data = generate_subspace_mog(n, intrinsic_dim=intrinsic_dim, ambient_dim=ambient_dim, seed=run)
            true_data = generate_subspace_mog(2000, intrinsic_dim=intrinsic_dim, ambient_dim=ambient_dim, seed=run)
    ...
    plt.tight_layout()
    plt.savefig("exp1_mog.pdf", dpi=300)
    print("\nSaved as 'exp1_mog.pdf'")
    plt.show()
